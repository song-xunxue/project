...
class SiameseNetwork(nn.Module):
    def __init__(self, cnn_gru_net):
        ...
        # 修正：输入维度改为128（匹配特征向量维度）
        self.fc_similarity = nn.Sequential(
            nn.Linear(128, 64),  # 输入是特征向量的维度128
            nn.ReLU(inplace=True),
            nn.Linear(64, 1)
        )
    ...
    def forward(self, x1, x2):
        # 提取特征向量
        out1 = self.cnn_gru_net(x1)  # (batch, 128)
        out2 = self.cnn_gru_net(x2)  # (batch, 128)
        # 计算特征绝对差，然后输入全连接层
        diff = torch.abs(out1 - out2)  # (batch, 128)
...
def get_next_filename(base_name, extension, start_num=1, digit_width=2):
    """
    获取下一个自增长文件名
...
# 使用示例
base_name = "model"  # 可以改为 "output" 或其他你需要的名称
extension = "pt"  # 可以改为 "csv" 或其他你需要的扩展名

next_filename = get_next_filename(base_name, extension)
# 主函数
def main():
    # 设置设备
    ...
    save_file=get_next_filename('submission','csv')
    submission_df.to_csv(save_file, index=False)
    print("=====预测完成=====")
    print(f"预测结果已保存到 submission.csv，共 {len(predictions)} 个预测")
